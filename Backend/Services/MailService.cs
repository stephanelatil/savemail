...
public class MailService : IMailService
{
    ...
    internal async Task<EmailAddress> GetOrCreateEmailAddresses(EmailAddress address)
    {
    ...
    internal async Task HandleEmailAddresses(Mail mail)
    {
    ...
    internal async Task InsertReply(Mail reply, Mail? parent)
    {
        if (parent is null)
            return;
        if (parent.HasReply && parent.Reply is not null)
            ...
            await this.InsertReply(reply,
                    //parent.reply may not have an ID yet if it's not in the db (must be in change tracker then).
                    //Use unique hashes for check
                    this._context.ChangeTracker.Entries<Mail>()
                                .Where(m => m.Entity.UniqueHash == parent.Reply.UniqueHash
                                        && m.Entity.UniqueHash2 == parent.Reply.UniqueHash2)
                                .FirstOrDefault()?.Entity
                    // Not in change tracker must be in DB thus have an ID
    ...
    internal async Task<List<Mail>> GetMailsToAdd(List<Mail> mails){
        ...
        foreach (var mail in mails)
        {
            foreach (var existingMail in existingMailsDict
                            .Where(m => mail.UniqueHash == m.UniqueHash && mail.UniqueHash2 == m.UniqueHash2))
            {
